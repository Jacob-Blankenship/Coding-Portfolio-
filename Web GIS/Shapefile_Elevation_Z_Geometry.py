...
while num1 < 5: # 5 is the amount of floors you have in the building
	...
	with arcpy.da.UpdateCursor("C:\\3D_Project\\All_Network_Floors-20180417T155721Z-001\\All_Network_Floors\\Floor_{}_R_P.shp".format(num), "SHAPE@Z", explode_to_points = True) as cursor:
		for row in cursor:
			row[0] = elev[num1]
			cursor.updateRow(row)
			
	
	print(str(row[0]) + " " + str(elev[num1])+ " " +str(num))
	num += 1
	num1 += 1
